...
func MSettingConnectionMessageSearchViewHandler(w http.ResponseWriter, r *http.Request) {
	common.WriteLog(config.INFO, "Start", r)
	// デフォルト遷移先
	tmpl, err := common.ViewParses("./auto/view/m_setting_connection_message/index.html")

	var resultDTO autodbdto.DBMSettingConnectionMessageResultDTO
	resultDTO.Token = common.GenerateUID()

	// POSTされてきたデータをFORMへ詰め込む
	form := new(autodbdto.DBMSettingConnectionMessageForm)
	...
	if form.Mode == config.BUTTON_BACK {
		...
		jsonBytes := ([]byte)(data.GetStringSession(r, "MSettingConnectionMessageSearchViewHandlerKey"))
	...
	if form.Mode == config.BUTTON_SEARCH {
		...
		if validaterErr != nil {
			...
			// 成功の場合にロジックを実行
			autoResult := autologic.SearchMSettingConnectionMessage(form, r)
			resultDTO.List = autoResult // 結果用DTOへ検索結果を格納
			tmpl, err = common.ViewParses("./auto/view/m_setting_connection_message/index.html")
			// 検索内容をセッションへ保持する
			jsonBytes, _ := json.Marshal(resultDTO.Form)
			data.SetStringSession(w, r, "MSettingConnectionMessageSearchViewHandlerKey", string(jsonBytes))
		}
	}
...
func MSettingConnectionMessageRegisterViewHandler(w http.ResponseWriter, r *http.Request) {
	common.WriteLog(config.INFO, "Start", r)
	// デフォルト遷移先
	tmpl, err := common.ViewParses("./auto/view/m_setting_connection_message/register.html")
	...
	if form.Mode == config.BUTTON_REGISTER {
		...
		if validaterErr != nil {
			...
			// 成功の場合にロジックを実行
			sid, errFlag := autologic.RegisterMSettingConnectionMessage(form, r)
			if errFlag == false {
				resultDTO.Status = config.RESULT_ERROR
			} else {
		...
		if form.Sid != ""  {
			resultForm, getted := autologic.GetByPkMSettingConnectionMessage(form, r) // 結果用DTOへFormを格納
