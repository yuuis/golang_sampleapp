...
func MIshiguroSearchViewHandler(w http.ResponseWriter, r *http.Request) {
	common.WriteLog(config.INFO, "Start", r)
	// デフォルト遷移先
	tmpl, err := common.ViewParses("./auto/view/m_ishiguro/index.html")

	var resultDTO autodbdto.DBMIshiguroResultDTO
	resultDTO.Token = common.GenerateUID()

	// POSTされてきたデータをFORMへ詰め込む
	form := new(autodbdto.DBMIshiguroForm)
	...
	if form.Mode == config.BUTTON_BACK {
		...
		jsonBytes := ([]byte)(data.GetStringSession(r, "MIshiguroSearchViewHandlerKey"))
	...
	if form.Mode == config.BUTTON_SEARCH {
		...
		// Validaterをかける
		validaterErr := common.GetValidate().StructExcept(form, "Sid", "Name", "Code", "DeleteFlag", "CreateUserId", "UpdateUserId", "CreateTime", "UpdateTime")
		if validaterErr != nil {
			// バリデーションエラーがある場合の遷移先
			...
			// 成功の場合にロジックを実行
			autoResult := autologic.SearchMIshiguro(form, r)
			resultDTO.List = autoResult // 結果用DTOへ検索結果を格納
			tmpl, err = common.ViewParses("./auto/view/m_ishiguro/index.html")
			// 検索内容をセッションへ保持する
			jsonBytes, _ := json.Marshal(resultDTO.Form)
			data.SetStringSession(w, r, "MIshiguroSearchViewHandlerKey", string(jsonBytes))
		}
	}
...
func MIshiguroRegisterViewHandler(w http.ResponseWriter, r *http.Request) {
	common.WriteLog(config.INFO, "Start", r)
	// デフォルト遷移先
	tmpl, err := common.ViewParses("./auto/view/m_ishiguro/register.html")
	...
	if form.Mode == config.BUTTON_REGISTER {
		...
		if validaterErr != nil {
			...
			// 成功の場合にロジックを実行
			sid, errFlag := autologic.RegisterMIshiguro(form, r)
			if errFlag == false {
				resultDTO.Status = config.RESULT_ERROR
			} else {
		...
		if form.Sid != ""  {
			resultForm, getted := autologic.GetByPkMIshiguro(form, r) // 結果用DTOへFormを格納
