...
func RFirstConversionSearchViewHandler(w http.ResponseWriter, r *http.Request) {
	common.WriteLog(config.INFO, "Start", r)
	// デフォルト遷移先
	tmpl, err := common.ViewParses("./auto/view/r_first_conversion/index.html")

	var resultDTO autodbdto.DBRFirstConversionResultDTO
	resultDTO.Token = common.GenerateUID()

	// POSTされてきたデータをFORMへ詰め込む
	form := new(autodbdto.DBRFirstConversionForm)
	...
	if form.Mode == config.BUTTON_BACK {
		...
		jsonBytes := ([]byte)(data.GetStringSession(r, "RFirstConversionSearchViewHandlerKey"))
	...
	if form.Mode == config.BUTTON_SEARCH {
		...
		// Validaterをかける
		validaterErr := common.GetValidate().StructExcept(form, "Sid", "MSettingSid", "UserKey", "FirstTBeaconSid")
		if validaterErr != nil {
			// バリデーションエラーがある場合の遷移先
			...
			// 成功の場合にロジックを実行
			autoResult := autologic.SearchRFirstConversion(form, r)
			resultDTO.List = autoResult // 結果用DTOへ検索結果を格納
			tmpl, err = common.ViewParses("./auto/view/r_first_conversion/index.html")
			// 検索内容をセッションへ保持する
			jsonBytes, _ := json.Marshal(resultDTO.Form)
			data.SetStringSession(w, r, "RFirstConversionSearchViewHandlerKey", string(jsonBytes))
		}
	}
...
func RFirstConversionRegisterViewHandler(w http.ResponseWriter, r *http.Request) {
	common.WriteLog(config.INFO, "Start", r)
	// デフォルト遷移先
	tmpl, err := common.ViewParses("./auto/view/r_first_conversion/register.html")
	...
	if form.Mode == config.BUTTON_REGISTER {
		...
		if validaterErr != nil {
			...
			// 成功の場合にロジックを実行
			sid, errFlag := autologic.RegisterRFirstConversion(form, r)
			if errFlag == false {
				resultDTO.Status = config.RESULT_ERROR
			} else {
		...
		if form.Sid != ""  {
			resultForm, getted := autologic.GetByPkRFirstConversion(form, r) // 結果用DTOへFormを格納
