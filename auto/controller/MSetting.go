...
func MSettingSearchViewHandler(w http.ResponseWriter, r *http.Request) {
	common.WriteLog(config.INFO, "Start", r)
	// デフォルト遷移先
	tmpl, err := common.ViewParses("./auto/view/m_setting/index.html")

	var resultDTO autodbdto.DBMSettingResultDTO
	resultDTO.Token = common.GenerateUID()

	// POSTされてきたデータをFORMへ詰め込む
	form := new(autodbdto.DBMSettingForm)
	...
	if form.Mode == config.BUTTON_BACK {
		...
		jsonBytes := ([]byte)(data.GetStringSession(r, "MSettingSearchViewHandlerKey"))
	...
	if form.Mode == config.BUTTON_SEARCH {
		...
		// Validaterをかける
		validaterErr := common.GetValidate().StructExcept(form, "Sid", "BusinessId", "MUserSid", "Key", "ActiveFlag", "UrlMatchingPattern", "Status", "DeleteFlag", "CreateUserId", "UpdateUserId", "CreateTime", "UpdateTime")
		if validaterErr != nil {
			// バリデーションエラーがある場合の遷移先
			...
			// 成功の場合にロジックを実行
			autoResult := autologic.SearchMSetting(form, r)
			resultDTO.List = autoResult // 結果用DTOへ検索結果を格納
			tmpl, err = common.ViewParses("./auto/view/m_setting/index.html")
			// 検索内容をセッションへ保持する
			jsonBytes, _ := json.Marshal(resultDTO.Form)
			data.SetStringSession(w, r, "MSettingSearchViewHandlerKey", string(jsonBytes))
		}
	}
...
func MSettingRegisterViewHandler(w http.ResponseWriter, r *http.Request) {
	common.WriteLog(config.INFO, "Start", r)
	// デフォルト遷移先
	tmpl, err := common.ViewParses("./auto/view/m_setting/register.html")
	...
	if form.Mode == config.BUTTON_REGISTER {
		...
		if validaterErr != nil {
			...
			// 成功の場合にロジックを実行
			sid, errFlag := autologic.RegisterMSetting(form, r)
			if errFlag == false {
				resultDTO.Status = config.RESULT_ERROR
			} else {
		...
		if form.Sid != ""  {
			resultForm, getted := autologic.GetByPkMSetting(form, r) // 結果用DTOへFormを格納
