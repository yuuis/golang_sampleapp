...
func MShopSearchViewHandler(w http.ResponseWriter, r *http.Request) {
	common.WriteLog(config.INFO, "Start", r)
	// デフォルト遷移先
	tmpl, err := common.ViewParses("./auto/view/m_shop/index.html")

	var resultDTO autodbdto.DBMShopResultDTO
	resultDTO.Token = common.GenerateUID()

	// POSTされてきたデータをFORMへ詰め込む
	form := new(autodbdto.DBMShopForm)
	...
	if form.Mode == config.BUTTON_BACK {
		...
		jsonBytes := ([]byte)(data.GetStringSession(r, "MShopSearchViewHandlerKey"))
	...
	if form.Mode == config.BUTTON_SEARCH {
		...
		// Validaterをかける
		validaterErr := common.GetValidate().StructExcept(form, "Sid", "ShopId", "BusinessId", "ManagerAuthKey", "DeleteFlag", "CreateUserId", "UpdateUserId", "CreateTime", "UpdateTime")
		if validaterErr != nil {
			// バリデーションエラーがある場合の遷移先
			...
			// 成功の場合にロジックを実行
			autoResult := autologic.SearchMShop(form, r)
			resultDTO.List = autoResult // 結果用DTOへ検索結果を格納
			tmpl, err = common.ViewParses("./auto/view/m_shop/index.html")
			// 検索内容をセッションへ保持する
			jsonBytes, _ := json.Marshal(resultDTO.Form)
			data.SetStringSession(w, r, "MShopSearchViewHandlerKey", string(jsonBytes))
		}
	}
...
func MShopRegisterViewHandler(w http.ResponseWriter, r *http.Request) {
	common.WriteLog(config.INFO, "Start", r)
	// デフォルト遷移先
	tmpl, err := common.ViewParses("./auto/view/m_shop/register.html")
	...
	if form.Mode == config.BUTTON_REGISTER {
		...
		if validaterErr != nil {
			...
			// 成功の場合にロジックを実行
			sid, errFlag := autologic.RegisterMShop(form, r)
			if errFlag == false {
				resultDTO.Status = config.RESULT_ERROR
			} else {
		...
		if form.Sid != ""  {
			resultForm, getted := autologic.GetByPkMShop(form, r) // 結果用DTOへFormを格納
