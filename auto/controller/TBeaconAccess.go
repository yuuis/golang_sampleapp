...
func TBeaconAccessSearchViewHandler(w http.ResponseWriter, r *http.Request) {
	common.WriteLog(config.INFO, "Start", r)
	// デフォルト遷移先
	tmpl, err := common.ViewParses("./auto/view/t_beacon_access/index.html")

	var resultDTO autodbdto.DBTBeaconAccessResultDTO
	resultDTO.Token = common.GenerateUID()

	// POSTされてきたデータをFORMへ詰め込む
	form := new(autodbdto.DBTBeaconAccessForm)
	...
	if form.Mode == config.BUTTON_BACK {
		...
		jsonBytes := ([]byte)(data.GetStringSession(r, "TBeaconAccessSearchViewHandlerKey"))
	...
	if form.Mode == config.BUTTON_SEARCH {
		...
		if validaterErr != nil {
			...
			// 成功の場合にロジックを実行
			autoResult := autologic.SearchTBeaconAccess(form, r)
			resultDTO.List = autoResult // 結果用DTOへ検索結果を格納
			tmpl, err = common.ViewParses("./auto/view/t_beacon_access/index.html")
			// 検索内容をセッションへ保持する
			jsonBytes, _ := json.Marshal(resultDTO.Form)
			data.SetStringSession(w, r, "TBeaconAccessSearchViewHandlerKey", string(jsonBytes))
		}
	}
...
func TBeaconAccessRegisterViewHandler(w http.ResponseWriter, r *http.Request) {
	common.WriteLog(config.INFO, "Start", r)
	// デフォルト遷移先
	tmpl, err := common.ViewParses("./auto/view/t_beacon_access/register.html")
	...
	if form.Mode == config.BUTTON_REGISTER {
		...
		if validaterErr != nil {
			...
			// 成功の場合にロジックを実行
			sid, errFlag := autologic.RegisterTBeaconAccess(form, r)
			if errFlag == false {
				resultDTO.Status = config.RESULT_ERROR
			} else {
		...
		if form.Sid != ""  {
			resultForm, getted := autologic.GetByPkTBeaconAccess(form, r) // 結果用DTOへFormを格納
