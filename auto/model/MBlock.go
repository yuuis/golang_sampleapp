 ...
 */ 
func GetByPKMBlock(sid string, r *http.Request) (autodbdto.DBMBlockDTO, bool, bool) { 
	db := db.DbConn() 
 
	sql := "select * from m_block where sid = ? "
	stmt, err := db.Prepare(sql) 
	if err != nil { 
		common.WriteErrorLog(config.FATAL, err, r) 
		var rv autodbdto.DBMBlockDTO 
	...
	defer rows.Close() 
	for rows.Next() { 
		var columns autodbdto.DBMBlockDTO 
	...
	var rdto autodbdto.DBMBlockDTO 
 ...
 * 取得できなかった場合は空の構造体とfalseを返す 
 */ 
func GetByPKMBlockForUpdate(db *sql.Tx, sid string, r *http.Request) (autodbdto.DBMBlockDTO, bool, bool) { 
 ...
 * 取得できなかった場合は空の配列を返す 
 */ 
func SelectByInMBlock(targetColumn string, in []string, r *http.Request) ([]autodbdto.DBMBlockDTO, bool) { 
	var rdto []autodbdto.DBMBlockDTO 
	...
	sql := "select * from m_block where " + targetColumn + " in (" 
	where := "" 
	for ind, _ := range in { 
		if where != "" { 
			where = where + "," 
 ...
 * 取得できなかった場合は空の配列を返す 
 */ 
func SelectByInMBlockForUpdate(db *sql.Tx, targetColumn string, in []string, r *http.Request) ([]autodbdto.DBMBlockDTO, bool) { 
	var rdto []autodbdto.DBMBlockDTO 
 ...
 * 取得できなかった場合は、空の配列を返す 
 */ 
func SelectMBlock(search *autodbdto.DBMBlockForm, r *http.Request) ([]autodbdto.DBMBlockDTO, bool) { 
	var rdto []autodbdto.DBMBlockDTO 
	...
	where, arr = AppendWhere(search.TargetNo, "target_no", where, arr) 
 ...
 * 成功時には追加したIDを返す 
 */ 
func InsertMBlock(db *sql.Tx, search *autodbdto.DBMBlockForm, r *http.Request, excludes []string) (int64, bool) { 
	columnSql := "" 
	valueSql := "" 
	var arr []string 
	columns := []string{ 
 ...
 * 除外したいカラムはexcludesへ配列で格納する 
 */ 
func UpdateMBlock(db *sql.Tx, search *autodbdto.DBMBlockForm, r *http.Request, excludes []string) bool { 
	columnSql := "" 
	var arr []string 
	columns := []string{ 
		"sid", 
 ...
 * バルクインサート 
 */ 
func BulkInsertMBlock(db *sql.Tx, search []*autodbdto.DBMBlockForm, r *http.Request, excludes []string) (int64, bool) { 
	columnSql := "" 
	valueSql := "" 
	var arr []string 
	columns := []string{ 
	...
	for dataInd, _ := range search { 
		...
		for ind, _ := range columns { 
			if IsColumnExcludes(columns[ind], excludes) == false { 
				...
				case 3: 
					arr = append(arr, search[dataInd].TargetNo) 
				case 4: 
 ...
 * バルクアップデート 
 */ 
func BulkUpdateMBlock(db *sql.Tx, search []*autodbdto.DBMBlockForm, r *http.Request, excludes []string) (int64, bool) { 
	columnSql := "" 
	valueSql := "" 
	duplicateKeys := "" 
...
func IsColumnExcludes(t string, excludes []string) bool { 
	for ind, _ := range excludes { 
		if excludes[ind] == t { 
			return true 
		} 
	} 
...
func AppendWhere(s string, c string, w string, arr []string) (string, []string) { 
	if s != "" { 
		if w != "" { 
			w = w + " and " 
		} 
